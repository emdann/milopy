...
@pytest.fixture
def anndata_subsample_samples(seed=42):
    adata = sc.datasets.pbmc68k_reduced()
    ...
    ## Simulate replicates ##
    adata.obs["replicate"] = np.random.choice(
        ["R1", "R2", "R3", 'R4', 'R5'], size=adata.n_obs)
    adata.obs["sample"] = adata.obs["replicate"] + adata.obs["condition"]
    count_nhoods(adata, sample_col="sample")
    return adata
...
def test_pvalues(anndata):
    ...
    min_p, max_p = nhood_adata.obs["PValue"].min(
    ), nhood_adata.obs["PValue"].min()
...
def test_fdr(anndata):
    ...
    DA_nhoods(adata, design="~condition")
    nhood_adata = adata.uns["nhood_adata"]
    assert np.all(np.round(nhood_adata.obs["PValue"], 10) <= np.round(
        nhood_adata.obs["SpatialFDR"], 10)), "FDR is higher than uncorrected P-values"


def test_fdr_select_samples(anndata_subsample_samples):
    adata = anndata_subsample_samples.copy()
    DA_nhoods(adata, design="~ condition", subset_samples=[
        "R1ConditionA", "R1ConditionB", "R3ConditionA", "R3ConditionB"])
    nhood_adata = adata.uns["nhood_adata"]
    np.all((np.round(nhood_adata.obs["PValue"], 10) <= np.round(nhood_adata.obs["SpatialFDR"], 10)) | (
        nhood_adata.obs["SpatialFDR"].isna())), "FDR is higher than uncorrected P-values"
